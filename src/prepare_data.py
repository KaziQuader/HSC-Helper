...
def data_pipeline(input_path, output_path, embedding_model):
...
def create_vector_db(input_path, output_path, embedding_model, url="http://localhost:6333", collection_name='hsc_helper'):
    qdrant = QdrantClient(url)

    if collection_name not in [col.name for col in qdrant.get_collections().collections]:
        payload, embeddings, embedding_dim = data_pipeline(input_path, output_path, embedding_model)
